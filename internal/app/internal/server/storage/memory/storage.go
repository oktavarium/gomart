...
func NewStorage() *storage {
	return &storage{users: make(map[string]shared.User)}
}
...
func (s *storage) UserInfo(u shared.User) (shared.UserInfo, error) {
	exists, err := s.UserExists(u)
	if err != nil {
		return shared.UserInfo{}, fmt.Errorf("error on checking user existance: %w", err)
	}

	if !exists {
		return shared.UserInfo{}, fmt.Errorf("user not exists: %w", err)
	}
